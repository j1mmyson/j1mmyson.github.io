...
그러면 너는  ` ```mermaid`로 그래프 코드를 감싸 다른 마크다운 언어와 같이 사용할 수 있다.
...
``````language``` 를 통해 line number 와 syntax highlight를 갖는 코드블럭을 생성할 수 있다.

> **Note**: ``` {% highlight LANGUAGE %}```나 ```{% highlight LANGUAGE linenos %}```는 이 테마에서 사용할 수 없다!
...
만약 **Liquid** snippet(수도코드?)를 배치하고싶다면 liquid code를 ```{% raw %}```와 ```{% endraw%}```로 감싸주면된다.
